...
class TfIdfAll:
    ...
    def base_tfidf_feature(self, path, path2):
        ...
        documents2 = pd.read_table(path2)['EVT']    #  .values.astype('U')
        documents_all = pd.concat([documents, documents2], axis=0)
        print("documents : ", documents.shape)
        print("documents2 : ", documents2.shape)
        print("documents_all : ", documents_all.shape)
        # print(documents)
        ...
        tfv_result = tfv.fit_transform(documents_all).toarray()
        print("tfv_result")
    ...
    def save_ftidf_feature_csv(self):
        ...
        read_data2 = self.data.output.prep_evt_have_log_usr_test

        save_feature = self.data.feature.tf_idf_have_log_usr_evt_all

        # names, tfv_result = self.base_tfidf_feature(read_data)
        pd_dataframe = self.base_tfidf_feature(read_data, read_data2)
